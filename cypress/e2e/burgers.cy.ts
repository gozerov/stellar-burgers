...
const FILLING = `[data-cy="643d69a5c3f7b9001cfa0941"]`;
...
describe("Работа с ингредиентами в заказе", () => {
	it("увеличение счётчика при добавлении начинки", () => {
	...
	it("замена булки без начинки", () => {
		cy.get(BUN_A).find("button").click();
		cy.get(`[data-cy='constructor-pos-top']`).should("contain", "Краторная булка N-200i");
		cy.get(BUN_B).find("button").click();
		cy.get(`[data-cy='constructor-pos-top']`).should("contain", "Флюоресцентная булка R2-D3");
	});
